...
// src/schema.ts
var signupInput = external_exports.object({
  name: external_exports.string(),
  email: external_exports.string().email(),
  password: external_exports.string().min(6)
});
var signinInput = external_exports.object({
  email: external_exports.string().email(),
  password: external_exports.string()
});
var blogInput = external_exports.object({
  title: external_exports.string(),
  content: external_exports.string()
});
var updateblogInput = external_exports.object({
...
userRouter.post("/signup", async (c) => {
  ...
  try {
    const result = await sql`
      ...
      VALUES (${parsed.data.email}, ${parsed.data.password}, ${parsed.data.name})
      RETURNING id
    `;
    const user = result[0];
...
blogRouter.post("/", async (c) => {
  ...
  try {
    const result = await sql`
        ...
        VALUES (${parsed.data.title}, ${parsed.data.content}, ${authorId})
        RETURNING *
    `;
    return c.json({
      message: "Blog added!",
...
// .wrangler/tmp/bundle-wbOYrq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-wbOYrq/middleware-loader.entry.ts
